...
Let $X \in \mathbb{R}^{N \times D}$ be the matrix of input patch embeddings, where:
- $N = 196$ → number of patches  
...
For each head $i \in \{1, \dots, 12\}$, the input $X$ is projected into Queries, Keys, and Values:
...
**Explanation:**
- $Q_i K_i^T$ → similarity between patches  
- $\sqrt{d_k}$ → scaling to stabilize gradients  
...
$$
\text{MultiHead}(Q, K, V) =
\text{Concat}(\text{head}_1, \dots, \text{head}_{12}) W^O
$$
...
### Why it matters for MRI

Medical diagnosis depends on multiple factors like size, texture, and spatial relationships.  
Multi-Head Self-Attention (MHSA) allows the model to:

- Capture global dependencies  
- Avoid focusing on a single feature  
- Learn richer representations for tumor classification  
...
This project is intended for **research and educational purposes only**. The predictions generated by the models are not a substitute for professional medical advice, diagnosis, or treatment. Always seek the advice of a qualified healthcare provider with any questions regarding a medical condition. Never disregard professional medical advice or delay in seeking it because of something you have read here.

---

## 🤝 Contributing

Contributions are welcome! If you have ideas for improving the classification accuracy, optimizing the models, or adding new explainability features:
...
*Brain Tumor MRI Classifier · Advancing Medical Imaging with Vision Transformers*
